...
RESEARCHER (gemini-2.5-pro, ReAct agent + 7 search tools: 6 Tavily + search_google_live)
  │  ≥30 search calls (phases sum ~30-38) incl. per-startup latest-news FRESHNESS pass
...
│   │     test_captable.py (30) · test_call_claims.py (26) · test_delta.py (29) · test_clerk_auth.py (14) · test_tour.py (59) · test_debate_log.py (35) · test_protocol_guard.py (22) — 710 total (test_freshness now 66, test_trust now 79); run from REPO ROOT
│   └── app/
...
└── frontend/
    ...
    └── src/
        ...
        └── components/
            ...
            ├── HistoryList.tsx          ← inline History table on /app Home: search, star, rename, re-run, delete, reopen (replaced the old HistoryDrawer)
...
- **Research protocol** (`TOOL_CHOREOGRAPHY_INSTRUCTIONS`, researcher only): ≥30 Tavily calls in 4 phases (market+sizing → per-startup financials/moat **+ a mandatory `search_latest_news` freshness pass per startup** → regulatory+founders → exit/M&A), enforce **≥6 (ideally 8) startups** + name 2–3 **incumbents** as map reference, widen to adjacent stages when <6 pure-plays exist (and say so), mark missing data "Not Disclosed", never fabricate.
...
- **Research observability + freshness audit (added 2026-07-02).** `researcher_node` no longer discards the ReAct transcript blindly: `_research_manifest` counts every tool call (total / by-tool / failed) → `final_report.research_manifest` + a rich agent log with a **PROTOCOL SHORTFALL** flag; **PROTOCOL GUARD (2026-07-24, in-code):** if the first attempt makes <`PROTOCOL_MIN_TOOL_CALLS` (30) calls, `researcher_node` re-runs the researcher ONCE with a violation notice appended and keeps the better attempt by call count (`manifest.protocol_retries` discloses it; ties keep the original) — added after a live zero-call run wrote its brief from memory in 80s; `_harvest_source_index` pulls the REAL source URLs out of the tool transcripts into an auto-generated "Source Index" appended to `research_data` (kills compiler-fabricated Works Cited links — observed live 2026-07-02); `_data_freshness` audits the compiled report's dated mentions (newest/oldest/lag) → `final_report.data_freshness` + a "Data as of" badge in `ReportViewer` (amber when newest evidence >6 months old); every Tavily/grounded call is now logged (live tail during the researcher phase). The compiler is told to close §0 with "*Research data as of <date>*" and to never invent URLs.
...
- **History (durable store).** Every finished run is persisted by the worker (save-on-completion in `tasks.py`) to `services/store.py` on the `reports` volume (`<reports_dir>/<id>.json` = light meta + full `final_report`). Survives restarts and the Redis purge. CRUD: `GET /api/reports` (light list, starred-first-then-newest), `GET /api/reports/{id}` (full), `PATCH` (label/star), `DELETE`. UI: `HistoryList.tsx` — the "Recent analyses" table rendered INLINE on the /app Home view (the old HistoryDrawer slide-out was removed; Home IS the history now) with **search**, **star favorites**, **rename/label**, **re-run**, **delete** (row actions always visible, not hover-only), and **click-to-reopen** a past run in `ReportViewer`. The /app console is a client-side view state machine in `app/app/page.tsx` (`home | sector | vc | founder`, deep-linkable via `/app?view=…`): Home = greeting + stat strip + mode cards + inline History; the three mode views render `ResearchForm` preconfigured via its `mode` prop (sector = focal panel hidden, exact focal-off payload; vc/founder = `FocalStartupPanel` pinned on with switch + mode toggle hidden). `ConsoleShell` takes `active` + `onNavigate` (no hardcoded workspace links); the wordmark and "Home" both return home. **Two more workspace views (2026-08-02):** `fund` — the **Fund Profile** (lib/fundProfile.ts, localStorage `prospectus-fund-profile`): set fund economics once, every form auto-applies them (the 05 card collapses to an "applied" summary with per-run adjust/opt-out that NEVER writes back); and `tracked` — **Tracked** (`TrackedList.tsx`): longitudinal chains (baseline + its re-runs) with the latest re-run's `run_delta` summary + prediction-audit tallies; powered by two derived light-list fields added to `_meta()` in `services/store.py` (`baseline_report_id` from `final_report.baseline_id`, `has_delta`) — no save-path change; chain ↻ re-runs the BASELINE (keeps one chain, keeps grading the original's predictions).
...
- **There ARE token-free tests now** (`backend/tests/`, 710 checks). Run them after backend changes (they stub LLM deps — no keys/cost). But there's still **no `.git`** and no live-run CI — for behavior/quality, also run the stack and exercise a real request. **Rebuild the image after code changes** (source is baked).
