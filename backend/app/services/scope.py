...
MARKET-ANALYSIS PROMPT so the user doesn't have to type them. Strategy:
  ...
  - uploaded materials present  -> infer from the materials (no search needed)
...
def infer_scope(focal_startup: str, focal_upload_id: str = "") -> dict:
    ...
    source is 'materials' | 'search' | 'none'. autoderived is False if inference failed
    (caller then keeps whatever the user typed).
    """
    ...
    # 2) No materials -> ground the inference with a couple of web searches on the name.
    if not context.strip() and focal:
        try:
